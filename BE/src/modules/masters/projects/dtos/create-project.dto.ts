...
export class CreateProjectDto {
  ...
  @Matches(DECIMAL_REGEX, {
    message:
      'reraRegularization must be a decimal with three digits after the decimal point and between 0.010 and 100',
  })
  ...
  @Transform(({ value }) => (value?.trim() === '' ? null : value))
  @ValidateIf(
    (o) => o?.reraPayable !== undefined && o?.reraPayable?.trim() !== '',
  )
  @Matches(DECIMAL_REGEX, {
    message:
      'reraPayable must be a decimal with three digits after the decimal point and between 0.010 and 100',
  })
  reraPayable: string;
  ...
  @Matches(DECIMAL_REGEX, {
    message:
      'rtmRegularization must be a decimal three digits after the decimal point and between 0.010 and 100',
  })
  ...
  @Transform(({ value }) => (value?.trim() === '' ? null : value))
  @ValidateIf(
    (o) => o?.rtmPayable !== undefined && o?.rtmPayable?.trim() !== '',
  )
  @Matches(DECIMAL_REGEX, {
    message:
      'rtmPayable must be a decimal with three digits after the decimal point and between 0.010 and 100',
  })
  rtmPayable: string;
  ...
  @IsEnum(PaymentGatewayEnum, {
    each: true,
    message: 'Each gateway must be Razorpay or Easebuzz',
  })
