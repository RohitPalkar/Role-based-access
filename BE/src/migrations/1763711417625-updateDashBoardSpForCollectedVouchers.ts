...
export class UpdateDashBoardSpForCollectedVouchers1763711417625 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`DROP FUNCTION IF EXISTS format_indian_amount;`);
    await queryRunner.query(`
CREATE  FUNCTION format_indian_amount(amount BIGINT) RETURNS varchar(50) CHARSET utf8mb4
    DETERMINISTIC
BEGIN
    ...
    IF amount >= 10000000 THEN 
        -- Crores
        SET result = CONCAT(ROUND(amount / 10000000, 2), ' Cr');

    ELSEIF amount >= 100000 THEN      
        -- Lakhs
        SET result = CONCAT(ROUND(amount / 100000, 2), ' Lakh');

    ELSEIF amount >= 1000 THEN        
        -- Thousands
        SET result = CONCAT(ROUND(amount / 1000, 2), 'K');

    ELSE  
        -- Below 1000 → just return number
        SET result = amount;
    END IF;
...
`);
    ...
    await queryRunner.query(`
CREATE PROCEDURE rm_dashboard_listing(IN input JSON)
...
SET @baseVoucherQuery = ' SELECT 
    ...
    WHERE 1=1 AND v.campaign_id is not null AND ec.status IN ("Active | Voucher","Active | EOI","Active | Voucher to EOI")';
SET @baseSql = '
   SELECT 
...
SET @limitQuery = '';
        ...
        SET @sql = CONCAT(
            ...
            FROM (
                SELECT 
                    ...
                    SUM(v.voucher_status NOT IN("Form Link Shared","Form fill in progress")) AS collectedEoiCount,
                    SUM(v.voucher_status IN("Form fill in progress")) AS inProgressEoiCount,
                       SUM(
        CASE 
            WHEN v.voucher_status NOT IN ("Form Link Shared","Form fill in progress") 
    ...
    SUM(
        ...
        END
                    ...
                    SUM(v.voucher_status IN ("Form Submitted","MIS-Resubmission requested","Updated as per MIS")) AS pendingMISCount,
                    SUM(v.voucher_status IN ("MIS Verified","CRM-Resubmission requested","Updated as per CRM")) AS pendingCRMCount,
                    SUM(v.voucher_status IN( "CRM Verified","MIS Verified","CRM-Resubmission requested","Updated as per CRM")) AS pendingFINCount,
                    SUM(((v.voucher_status IN ("MIS-Resubmission requested","CRM-Resubmission requested","Form Submitted","Updated as per CRM","Updated as per MIS")) OR (v.finance_status IN("Rejected", "Not Realized")))) AS pendingRMCount,
                    SUM(v.voucher_status = "Cancellation Requested") AS requested,
                    SUM(v.voucher_status = "Cancelled") AS processed,
                    SUM(v.voucher_status IN ("Cancellation Request Accepted","Cancellation Approved","Refund Initiated")) AS inProgress
                FROM eoi_campaigns ec
                LEFT JOIN vouchers v ON v.campaign_id = ec.id
                WHERE  ec.id IN (', @campaignIdList, ') AND v.id IN (' , @voucherIdList,' )
                GROUP BY ec.id
...
"purvaChampion", CONCAT_WS(" | ",
    ...
    SUM(
        ...
        END
            ...
            SUM(v.primary_source = "Channel Partner" AND v.voucher_status NOT IN("Form Link Shared","Form fill in progress")) AS channelpartner,
            SUM(v.primary_source IN ("Purva Privilege", "Provident Premier") AND v.voucher_status NOT IN("Form Link Shared","Form fill in progress")) AS loyalty,
            SUM(v.primary_source = "Digital Marketing" AND v.voucher_status NOT IN("Form Link Shared","Form fill in progress")) AS digital,
            SUM(v.primary_source = "Direct Walk-in" AND v.voucher_status NOT IN("Form Link Shared","Form fill in progress")) AS direct,
            SUM(v.primary_source = "Purva Champion" AND v.voucher_status NOT IN("Form Link Shared","Form fill in progress")) AS purvaChampion,
            SUM(CASE WHEN v.primary_source = "Channel Partner" AND v.voucher_status NOT IN("Form Link Shared","Form fill in progress")  THEN IFNULL(JSON_EXTRACT(v.payment_details,"$.amountPayable"),0) ELSE 0 END) AS total_channelpartner,
            SUM(CASE WHEN v.primary_source IN ("Purva Privilege","Provident Premier") AND v.voucher_status NOT IN("Form Link Shared","Form fill in progress")  THEN IFNULL(JSON_EXTRACT(v.payment_details,"$.amountPayable"),0) ELSE 0 END) AS total_loyalty,
            SUM(CASE WHEN v.primary_source = "Digital Marketing" AND v.voucher_status NOT IN("Form Link Shared","Form fill in progress") THEN IFNULL(JSON_EXTRACT(v.payment_details,"$.amountPayable"),0) ELSE 0 END) AS total_digital,
            SUM(CASE WHEN v.primary_source = "Direct Walk-in" AND v.voucher_status NOT IN("Form Link Shared","Form fill in progress") THEN IFNULL(JSON_EXTRACT(v.payment_details,"$.amountPayable"),0) ELSE 0 END) AS total_direct,
            SUM(CASE WHEN v.primary_source = "Purva Champion" AND v.voucher_status NOT IN("Form Link Shared","Form fill in progress") THEN IFNULL(JSON_EXTRACT(v.payment_details,"$.amountPayable"),0) ELSE 0 END) AS total_purvaChampion,
            SUM(CASE WHEN v.primary_source = "Channel Partner" AND v.voucher_status NOT IN("Form Link Shared","Form fill in progress") THEN IFNULL(JSON_EXTRACT(v.payment_details,"$.totalAmountPaid"),0) ELSE 0 END) AS paid_channelpartner,
            SUM(CASE WHEN v.primary_source IN ("Purva Privilege","Provident Premier") AND v.voucher_status NOT IN("Form Link Shared","Form fill in progress") THEN IFNULL(JSON_EXTRACT(v.payment_details,"$.totalAmountPaid"),0) ELSE 0 END) AS paid_loyalty,
            SUM(CASE WHEN v.primary_source = "Digital Marketing" AND v.voucher_status NOT IN("Form Link Shared","Form fill in progress") THEN IFNULL(JSON_EXTRACT(v.payment_details,"$.totalAmountPaid"),0) ELSE 0 END) AS paid_digital,
            SUM(CASE WHEN v.primary_source = "Direct Walk-in" AND v.voucher_status NOT IN("Form Link Shared","Form fill in progress") THEN IFNULL(JSON_EXTRACT(v.payment_details,"$.totalAmountPaid"),0) ELSE 0 END) AS paid_direct,
            SUM(CASE WHEN v.primary_source = "Purva Champion" AND v.voucher_status NOT IN("Form Link Shared","Form fill in progress") THEN IFNULL(JSON_EXTRACT(v.payment_details,"$.totalAmountPaid"),0) ELSE 0 END) AS paid_purvaChampion
        FROM eoi_campaigns ec
        LEFT JOIN vouchers v ON v.campaign_id = ec.id
        WHERE ec.id IN (', @campaignIdList, ') AND v.id IN (' , @voucherIdList,' )
        GROUP BY ec.id
