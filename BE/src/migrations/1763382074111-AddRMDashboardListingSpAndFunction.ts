...
export class AddRMDashboardListingSpAndFunction1763382074111 implements MigrationInterface {
...
"purvaChampion", CONCAT_WS(" | ",
    ...
    SUM(
        ...
        END
            ...
            SUM(v.primary_source = "Channel Partner") AS channelpartner,
            SUM(v.primary_source IN ("Purva Privilege", "Provident Premier")) AS loyalty,
            SUM(v.primary_source = "Digital Marketing") AS digital,
            SUM(v.primary_source = "Direct Walk-in") AS direct,
            SUM(v.primary_source = "Purva Champion") AS purvaChampion,
            SUM(CASE WHEN v.primary_source = "Channel Partner" THEN IFNULL(JSON_EXTRACT(v.payment_details,"$.amountPayable"),0) ELSE 0 END) AS total_channelpartner,
            SUM(CASE WHEN v.primary_source IN ("Purva Privilege","Provident Premier") THEN IFNULL(JSON_EXTRACT(v.payment_details,"$.amountPayable"),0) ELSE 0 END) AS total_loyalty,
            SUM(CASE WHEN v.primary_source = "Digital Marketing" THEN IFNULL(JSON_EXTRACT(v.payment_details,"$.amountPayable"),0) ELSE 0 END) AS total_digital,
            SUM(CASE WHEN v.primary_source = "Direct Walk-in" THEN IFNULL(JSON_EXTRACT(v.payment_details,"$.amountPayable"),0) ELSE 0 END) AS total_direct,
            SUM(CASE WHEN v.primary_source = "Purva Champion" THEN IFNULL(JSON_EXTRACT(v.payment_details,"$.amountPayable"),0) ELSE 0 END) AS total_purvaChampion,
            SUM(CASE WHEN v.primary_source = "Channel Partner" THEN IFNULL(JSON_EXTRACT(v.payment_details,"$.totalAmountPaid"),0) ELSE 0 END) AS paid_channelpartner,
            SUM(CASE WHEN v.primary_source IN ("Purva Privilege","Provident Premier") THEN IFNULL(JSON_EXTRACT(v.payment_details,"$.totalAmountPaid"),0) ELSE 0 END) AS paid_loyalty,
            SUM(CASE WHEN v.primary_source = "Digital Marketing" THEN IFNULL(JSON_EXTRACT(v.payment_details,"$.totalAmountPaid"),0) ELSE 0 END) AS paid_digital,
            SUM(CASE WHEN v.primary_source = "Direct Walk-in" THEN IFNULL(JSON_EXTRACT(v.payment_details,"$.totalAmountPaid"),0) ELSE 0 END) AS paid_direct,
            SUM(CASE WHEN v.primary_source = "Purva Champion" THEN IFNULL(JSON_EXTRACT(v.payment_details,"$.totalAmountPaid"),0) ELSE 0 END) AS paid_purvaChampion
        FROM eoi_campaigns ec
        LEFT JOIN vouchers v ON v.campaign_id = ec.id
        WHERE ec.id IN (', @campaignIdList, ') AND v.id IN (' , @voucherIdList,' )
        GROUP BY ec.id
