...
export function useTable(props?: UseTableProps): UseTableReturn {
  ...
  const onToggleColumnVisibility = useCallback((columnId: string) => {
  ...
  const onSetColumnVisibility = useCallback((columnId: string, visible: boolean) => {
    setVisibleColumns((prev) => ({
      ...prev,
      [columnId]: visible,
    }));
  ...
  const onSetAllColumnsVisibility = useCallback((visible: boolean, columnIds: string[]) => {
  ...
  const isColumnVisible = useCallback(
    (columnId: string, defaultVisible = true) => visibleColumns[columnId] === undefined ? defaultVisible : visibleColumns[columnId],
    [visibleColumns]
  );
