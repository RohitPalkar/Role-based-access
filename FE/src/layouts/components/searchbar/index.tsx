...
export function Searchbar({ data: navItems = [], sx, ...other }: SearchbarProps) {
  ...
  return (
    <>
      ...
      >
        <Box sx={{ p: 2, borderBottom: `solid 1px ${theme.vars.palette.divider}` }}>
          <InputBase
            fullWidth
            autoFocus
            placeholder="Search..."
            ...
            endAdornment={<Label sx={{ letterSpacing: 1, color: 'text.secondary', fontSize: '0.75rem' }}>esc</Label>}
            inputProps={{ sx: { typography: 'body1', padding: '8px 0' } }}
            sx={{
              minHeight: '36px',
              '& .MuiInputBase-input': {
                padding: '8px 0',
