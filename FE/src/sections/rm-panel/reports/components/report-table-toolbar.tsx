...
export function ReportsTableToolbar({ filters, options, onResetPage, isAdminReport }: Props) {
  ...
  const FY_OPTIONS = useMemo(() => [
    {
      label: `FY ${prevFYStartYear}-${String(prevFYStartYear + 1).slice(-2)} (April ${String(prevFYStartYear).slice(-2)} - March ${String(prevFYStartYear + 1).slice(-2)})`,
      value: 'previous'
    },
    { label: 'Current FY', value: 'current' },
  ], [prevFYStartYear]);

  const [selectedFy, setSelectedFy] = useState<string>('current');
  ...
  // Transform RM list to match CustomAutocomplete format
  ...
  const handleStartDateChange = useCallback(
    (date: Dayjs | null) => {
      setStartDate(date);
      setError(null);
      // Month/year pickers: compare by calendar month (same month must be valid).
  ...
  const handleFyChange = useCallback(
    (newValue: string | number | (string | number)[] | null) => {
      ...
      setSelectedFy(value || 'current');
      setError(null);
      if (value === 'previous') {
        setStartDate(dayjs(`${prevFYStartYear}-04-01`));
        setEndDate(dayjs(`${currentFYStartYear}-03-31`));
      } else {
        setStartDate(null);
        setEndDate(null);
      }
    ...
    [onResetPage, prevFYStartYear, currentFYStartYear]
  );
                ...
                <Grid item xs={12} sm={6} md={6} lg={6} xl={6}>
                  <FormControl fullWidth>
                    <DatePicker
                      ...
                      minDate={dayjs(`${currentFYStartYear}-04-01`)}
                      maxDate={dayjs()}
                      slotProps={{
                        textField: {
                          fullWidth: true,
                ...
                <Grid item xs={12} sm={6} md={6} lg={6} xl={6}>
                  <FormControl fullWidth>
                    <DatePicker
                      ...
                      minDate={startDate || dayjs(`${currentFYStartYear}-04-01`)}
                      maxDate={
                        startDate
                          ? (() => {
