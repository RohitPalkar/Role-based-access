...
/** `TextField` slot: clicking the input opens the picker (not only the icon). Lives here to avoid theme ↔ components circular imports. */
const DatePickerTextFieldOpenOnClick = React.forwardRef<HTMLDivElement, TextFieldProps>(
  (props, ref) => {
    const { onOpen, open } = usePickersContext();
    // `onClick` from the field is handleInputClick — omitted so we don't chain it (see below).
    const { onClick: onMouseDown, InputProps, disabled, inputProps, ...rest } =
      props;

    const openIfClosed = (e: React.UIEvent) => {
      if (disabled) return;
      if (open) return;
      onOpen(e);
    };
    ...
    return (
      <TextField
        ...
        onMouseDown={(e) => {
          onMouseDown?.(e);
          // Avoid caret / text-input flash: read-only field should not take focus on click.
          if (!disabled) {
            e.preventDefault();
          }
        }}
        onClick={(e) => {
          // Do not call MUI field `onClick` (handleInputClick): it runs syncSelectionFromDOM(),
          // which selects a date section / placeholder and forces a second click to clear it.
          openIfClosed(e);
        }}
        InputProps={{
          ...InputProps,
...
/** Icons + text field that opens the picker when the input is clicked (not only the calendar icon). */
const dateFieldSlots = {
  ...dateIconSlots,
  textField: DatePickerTextFieldOpenOnClick,
};

const timeFieldSlots = {
  ...timeIconSlots,
  textField: DatePickerTextFieldOpenOnClick,
};
...
const MuiDatePicker: Components<Theme>['MuiDatePicker'] = {
   ...
   * DEFAULT PROPS
   *************************************** */
  defaultProps: { slots: dateFieldSlots },
};
...
const MuiStaticDatePicker: Components<Theme>['MuiStaticDatePicker'] = {
   ...
   * DEFAULT PROPS
   *************************************** */
  defaultProps: { slots: dateIconSlots },
};
...
const MuiTimePicker: Components<Theme>['MuiTimePicker'] = {
   ...
   * DEFAULT PROPS
   *************************************** */
  defaultProps: { slots: timeFieldSlots },
};
...
const MuiStaticTimePicker: Components<Theme>['MuiStaticTimePicker'] = {
   ...
   * DEFAULT PROPS
   *************************************** */
  defaultProps: { slots: timeIconSlots },
};
...
const MuiPickersPopper: Components<Theme>['MuiPickersPopper'] = {
  ...
  styleOverrides: {
    /**
     * Popover / filter panels use `theme.zIndex.modal` (1300). Without a higher z-index,
     * the calendar dropdown renders behind the filter popover (same layer, wrong paint order).
     */
    root: ({ theme }) => ({
